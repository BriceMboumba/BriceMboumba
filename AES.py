...
# La spécification Rijndael réelle inclut une taille de bloc variable,
# mais AES utilise une taille de bloc fixe de 16 octets (128 bits)

# De plus, AES permet une taille de clé variable,
# bien que cette implémentation d'AES n'utilise que des clés de chiffrement de 256 bits (AES-256)
...
# renvoie une copie du mot décalé de n octets (caractères) valeurs
# positives pour n octets décalés vers la gauche, valeurs négatives décalées vers la droite
def rotate(word, n):
    return word[n:] + word[0:n]


# itérer sur chaque ligne "virtuelle" dans la table d'état et décale les octets
# vers la GAUCHE par le décalage approprié
def shiftRows(state):
    for i in range(4):
        state[i * 4:i * 4 + 4] = rotate(state[i * 4:i * 4 + 4], i)


# itérer sur chaque ligne "virtuelle" de la table d'état et décale les octets à DROITE par le décalage approprié
def shiftRowsInv(state):
    for i in range(4):
        state[i * 4:i * 4 + 4] = rotate(state[i * 4:i * 4 + 4], -i)


# prend un mot de 4 octets et un numéro d'itération
def keyScheduleCore(word, i):
    # Faire pivoter un mot de 1 octet vers la gauche
    word = rotate(word, 1)
    newWord = []
    # appliquer la substitution sbox sur tous les octets de mot
    for byte in word:
        newWord.append(sbox[byte])
    # XOR la sortie de la transformation rcon [i] avec la première partie du mot
    newWord[0] = newWord[0] ^ rcon[i]
    return newWord


# développer la clé de chiffrement de 256 bits en une clé de 240 octets à partir de laquelle
# chaque clé est dérivée de la clé précédente par itération
...
# faire une transformation sbox sur chacune des valeurs de la table d'état (state table)
def subBytes(state):
...
# XOR chaque octet de roundKey avec la table d'état (state table)
...
# mixColumn prend une colonne et fait une boucle
...
# mixColumnInv fait également une boucle
...
# mixColumns est un support à mixColumn - génère une colonne "virtuelle" à partir de
# la table d'état (sate) et applique les coefficient de la matrice dans le corps de galois
def mixColumns(state):
    for i in range(4):
        column = []
        # créer la colonne en retirant le même élément de chaque ligne «virtuelle»
        for j in range(4):
            column.append(state[j * 4 + i])

        # applique mixColumn sur la colonne virtuelle
        mixColumn(column)

        # transférer les nouvelles valeurs dans la table d'état
        for j in range(4):
            state[j * 4 + i] = column[j]


# mixColumnsInv est un support à mixColumnInv - génère une colonne "virtuelle" à partir de
# la table d'état (sate) et applique les coefficient de la matrice dans le corps de galois
...
# aesRound applique chacune des 4 transformations dans l'ordre
...
# aesRoundInv applique chacune des 4 transformations inverse
...
# renvoie une clé ronde de 16 octets basée sur une clé développée et un numéro arrondi
def createRoundKey(expandedKey, n):
    return expandedKey[(n * 16):(n * 16 + 16)]


# créer une clé à partir d'un mot de passe fourni par l'utilisateur à l'aide de SHA-256
...
# fonction d'enveloppe (support) pour 14 tours d'AES, puisque nous utilisons une clé de 256 bits
...
# 14 tours d'inverse AES puisque nous utilisons une clé de 256 bits
...
# aesEncrypt - Chiffre un seul bloc du plaintext
...
# aesDecrypt - déchiffre un seul bloc du ciphertext
...
# renvoie un bloc de 16 octets à partir d'un fichier ouvert
# par bloc de 16 octets avec des caractères nuls si nécessaire
def getBlock(fp):
    raw = fp.read(16)
    # reached end of file
    if len(raw) == 0:
...
# encrypt - fonction d'enveloppe pour permettre le chiffrement de longueur arbitraire
# texte en clair utilisant le mode de retour de sortie (OFB)
def encrypt(myInput, password, outputfile=None):
    ...
    try:
        ...
        print
        "pyAES: unable to open input file -", myInput
        sys.exit()
    ...
    if outputfile is not None:
        try:
            ...
            print
            "pyAES: unable to open output file -", outputfile
            sys.exit()
    else:
        ...
        try:
            ...
            print
            "pyAES: unable to open output file -", filename
            sys.exit()
...
# decrypt - fonction d'enveloppe pour permettre le déchiffrement de longueur arbitraire
# texte chiffré utilisant le mode de retour de sortie (OFB)
def decrypt(myInput, password, outputfile=None):
    ...
    if outputfile is not None:
        ...
        if myInput[-4:] == ".aes":
            filename = myInput[:-4]
            print
            "Using", filename, "for output file name."
        else:
...
def printUsage():
    print
    "./pyAES.py [-e <input file> | -d <input file>] [(optional) -o <output file>]"
    print
    "You will be prompted for a password after you specify the encryption/decryption args.\n"
    sys.exit()


# rassemble les arguments de ligne de commande et valide l'entrée
def main():
    ...
    if inputfile is None:
        print
        "Error: please specify options for encryption or decryption."
        printUsage()
